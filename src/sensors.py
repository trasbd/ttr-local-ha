    ...
    try:
        if not hasattr(psutil, "sensors_fans"):
            raise NotImplementedError(
                "Platform not supported for sensors_fans")
...
def get_disk_usage(path):
    try:
        ...
        print('Error while trying to obtain disk usage from ' +
              str(path) + ' with exception: ' + str(e))
...
def get_zpool_use(pool):
    ...
    zpool_binary = shutil.which("zpool") or next(
        filter(lambda l: os.path.isfile(l), zpool_locations), None)
    try:
...
def get_net_data_tx(interface=True):
    ...
    if current_time == previous_time_tx:
        current_time += 1
    net_data = (current_net_data - old_net_data_tx) * 8 / \
        (current_time - previous_time_tx) / 1024
...
def get_net_data_rx(interface=True):
    ...
    if current_time == previous_time_rx:
        current_time += 1
    net_data = (current_net_data - old_net_data_rx) * 8 / \
        (current_time - previous_time_rx) / 1024
...
# subprocess.check_output(['/proc/net/wireless', 'grep wlan0'])
